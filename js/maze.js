...
var body = document.querySelectorAll("#play");
...
// makes the maze color
        context.fillStyle = open ? "#ffc654": "transparent";
...
//Changes the alert when you win the game
    function gameComplete() {
      ...
      var x = timeStop();
      document.getElementById("seconds1").style.display = "inline";
      document.getElementById("minutes1").style.display = "inline";
      document.getElementById("minutes1").innerHTML= x.substring(0,2);
      document.getElementById("colon").innerHTML= ":";
      console.log(x.substring(0,2));
      document.getElementById("seconds1").innerHTML= x.substring(3,5);
      console.log(x.substring(3,5));
      getScore(x);
        //alert('Yay you won!');
    }

    function getScore(play_score) {
      ...
      if(user){
        ...
        ref.on('value', function(snap){
          ...
          baseline_score = user_values.score;
          if (baseline_score){
            var baseline_strings = baseline_score.split(':');
            var baseline_seconds = (+baseline_strings[0]) * 60 + (+baseline_strings[1]);
            var play_strings = play_score.split(':');
            var play_seconds = (+play_strings[0]) * 60 + (+play_strings[1]);
            baseline_seconds = baseline_seconds *1.25;
            if(baseline_seconds > play_seconds){
            ...
            document.getElementById("zerobase").style.display = "block";
          }
        });
      }else{
        baseline_score = 120;
        var play_strings = play_score.split(':');
        var play_seconds = (+play_strings[0]) * 60 + (+play_strings[1]);
        if(baseline_score > play_seconds){
            document.getElementById("anonsober").style.display = "block";
        }
        else {
            document.getElementById("anondrunk").style.display = "block";
        }
      }
    }
    ...
    animate();
	//buttons.removeEventListener('click',handler);
  }
